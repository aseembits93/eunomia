...
def apply_operator(
    ...
    if operator_type == enums.ConditionOperator.EQUALS:
        return value == target
    if operator_type == enums.ConditionOperator.NOT_EQUALS:
        return value != target

    if isinstance(value, str) and isinstance(target, str):
        if operator_type == enums.ConditionOperator.CONTAINS:
            return value in target
        if operator_type == enums.ConditionOperator.NOT_CONTAINS:
            return value not in target
        if operator_type == enums.ConditionOperator.STARTS_WITH:
            return target.startswith(value)
        if operator_type == enums.ConditionOperator.ENDS_WITH:
            return target.endswith(value)

    elif isinstance(value, (int, float)) and isinstance(target, (int, float)):
        if operator_type == enums.ConditionOperator.GREATER:
            return value > target
        if operator_type == enums.ConditionOperator.GREATER_OR_EQUAL:
            return value >= target
        if operator_type == enums.ConditionOperator.LESS:
            return value < target
        if operator_type == enums.ConditionOperator.LESS_OR_EQUAL:
            return value <= target

    elif isinstance(value, list):
        if operator_type == enums.ConditionOperator.IN:
            return target in value
        if operator_type == enums.ConditionOperator.NOT_IN:
